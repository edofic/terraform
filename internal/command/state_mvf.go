...
func (c *StateMvfCommand) Run(args []string) int {
	args = c.Meta.process(args)
	// We create two metas to track the two states
	var backupPathOut, statePathOut string

	var dryRun bool
	cmdFlags := c.Meta.ignoreRemoteVersionFlagSet("state mvf")
	...
	// Read the config file bytes from configFilePath
	var moves []moveConfig
	configBytes, err := os.ReadFile(configFilePath)
	if err != nil {
		c.Ui.Error(fmt.Sprintf("Error reading config file: %s\n", err.Error()))
		return 1
	}
	for _, line := range strings.Split(string(configBytes), "\n") {
		...
		if len(move) != 2 {
			c.Ui.Error(fmt.Sprintf("Error parsing config file: %s\n", line))
			return 1
		}
		moves = append(moves, moveConfig{move[0], move[1]})
	}
	...
	for _, move := range moves {
		sourceAddr, moreDiags := c.lookupSingleStateObjectAddr(stateFrom, move.source)
		diags = diags.Append(moreDiags)
		destAddr, moreDiags := c.lookupSingleStateObjectAddr(stateFrom, move.dest)
		diags = diags.Append(moreDiags)
		if diags.HasErrors() {
			c.showDiagnostics(diags)
			return 1
		...
		for _, rawAddrFrom := range sourceAddrs {
			...
			case addrs.AbsResourceInstance:
				...
				moved++
				c.Ui.Output(fmt.Sprintf("%s %q to %q", prefix, addrFrom.String(), move.dest))
				if !dryRun {
...
func (c *StateMvfCommand) sourceObjectAddrs(state *states.State, matched addrs.Targetable) []addrs.Targetable {
...
func (c *StateMvfCommand) validateResourceMove(addrFrom, addrTo addrs.AbsResource) tfdiags.Diagnostics {
...
func (c *StateMvfCommand) Help() string {
	helpText := `
Usage: terraform [global options] state mvf [options] CONFIG_FILE

This command will move multiple items in the state based on the config file. 
The config file is read line by line and each line must follow the format

SOURCE_ADD DEST_ADDR
...
func (c *StateMvfCommand) Synopsis() string {
	return "Move multiple items in the state based on config file"
}
